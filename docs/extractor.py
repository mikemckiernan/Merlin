...
class SupportMatrixExtractor:
    ...
    def get_from_python(self, lookup: str, key=None):
        ...
        if err != 1:
            self.contdata[key] = str(output.decode("utf-8").strip())
        # Hacks
        if self.contdata[key].count("\n") > 0:
            self.contdata[key] = self.contdata[key].split("\n")[1]
...
def main(args):
    ...
    for cont in containers:
        ...
        with managed_container(img) as container:
            ...
            xtr.get_from_python("nvtabular")
            xtr.get_from_python("transformers4rec")
            xtr.get_from_python("merlin.core")
            xtr.get_from_python("merlin.systems")
            xtr.get_from_python("merlin.models")
            ...
            xtr.get_from_python("torch", "pytorch")
